...
def generate_pt_files(args, df):
    ...
    if not os.path.exists(temp_train_dir):
        ...
        for i in tqdm(range(len(df))):
            ...
            pt_file_path = os.path.join(temp_train_dir, os.path.splitext(feats_csv_path)[0].split("/")[-1] + ".pt")
...
def train(args, train_df, milnet, criterion, optimizer):
    ...
    for i, item in enumerate(dirs):
        ...
        if args.model == "psmil":
            bag_prediction, _, _, ins_prediction, _ = milnet(bag_feats, bag_label, bag_feats.shape[0], "psa", "TCGA")
    ...
    with torch.no_grad():
        for i, item in enumerate(test_df):
            ...
            if args.model == "psmil":
                ...
                ins_prediction, bag_prediction, _, _ = milnet(bag_feats)
                ins_prediction = torch.argmax(ins_prediction, 1)
                max_prediction, _ = torch.max(ins_prediction, 0)
                bag_loss = criterion(bag_prediction.view(1, -1), bag_label.view(1, -1))
                loss = bag_loss
            total_loss = total_loss + loss.item()
...
def main():
    ...
    parser.add_argument('--lr', default=0.001, type=float, help='Initial learning rate [0.0001]')
    ...
    parser.add_argument('--eval_scheme', default='5-fold-cv-standalone-test', type=str,
    ...
    def init_model(args):
        ...
        if args.model == "psmil":
            milnet = mil.PSMIL("TCGA", "no", False, 1).cuda()
            criterion = torch.nn.NLLLoss()
            optimizer = torch.optim.SGD(milnet.parameters(), lr=args.lr, momentum=0.9, weight_decay=1e-4)
            scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[5, 10, 15], last_epoch=-1)
        else:
    ...
    if args.eval_scheme == '5-fold-cv-standalone-test':
        ...
        bags_path = [path for path in glob.glob('temp_train/*.pt') if not os.path.basename(path).startswith('test')]
        bags_path = shuffle(bags_path)
        kf = KFold(n_splits=5, shuffle=True, random_state=42)
        fold_results = []
        fold_models = []
        ...
        if args.num_classes > 1:
            ...
            print("Testset Accuracy (Exact Match Ratio): Accuracy:", subset_accuracy,f"F1:{f1}" )
        else:
