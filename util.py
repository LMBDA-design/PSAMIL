...
def get_sampler(dataset_train):
    ...
    class_sample_count = np.asarray([10,2,2,1])
    weight = 1. / class_sample_count
    samples_weight = np.array([weight[t] for t in range(len(class_sample_count))])
...
def ICC(outs, targets):
    ...
    n = len(outs)
    mean_outs = torch.mean(outs)
    mean_targets = torch.mean(targets)

    BMS = n * torch.sum((mean_outs - outs) ** 2) / (n - 1)
    WMS = torch.sum((outs - targets) ** 2) / n

    ICC = (BMS - WMS) / (BMS + (n - 1) * WMS)
